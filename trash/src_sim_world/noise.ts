 ...
 *  Returns values in [0, 1] (terrain height: 0 = valley floor, 1 = ridge crest).
 *  Many noise libraries return [-1, 1]; this wrapper shifts and scales so callers
 *  get positive terrain heights without thinking about it.
 */
export function fbm(noise: SimplexNoise, x: number, y: number, octaves: number, lacunarity: number, gain: number): number {
  ...
  // normalize to [0, 1] terrain height: 0 = low, 1 = high.
