/**
 * Step 6 pipeline — single-file ESM runner (plain JS, no TS annotations).
 *
 * Run with:
 *   npx tsx tests/pipeline.mjs
 *   node --import tsx tests/pipeline.mjs
 *
 * Covers: meandering coast, region biases (bounded + biome-shift),
 * chunk determinism, seam height consistency, river carving on both sides
 * of a chunk seam, and varied terrain.
 */
...
// ---------- 1. Region bias: nudges, never overrides ----------
...
// ---------- 2. Moisture bias actually shifts biome classification ----------
...
// ---------- 3. Full ChunkManager pipeline: coast + rivers + regions ----------
...
// ---------- 4. Chunk-boundary consistency: no seams ----------
...
// ---------- 5. River carving continuity across chunk seams ----------
...
check(
  ...
  synthetic.tiles[8][8][10].terrainType === 'water' &&
    synthetic.tiles[8][8][9].terrainType === 'dirt',
);
check(
  'air tiles untouched',
  synthetic.tiles[8][8][11].terrainType === 'grass' &&
    synthetic.tiles[8][8][11].elevation === -1,
);
...
if (failures.size > 0) {
  ...
  console.log('\nAll pipeline mjs checks passed.');
}
